...
public class Tiger extends Enemy  implements JumpListener {
	...
	public Tiger(double x, double y, Canvas instance) {
		...
		cout =0;
		try {
			img = ImageIO.read(new File("resProj/Character/tiger.png"));
		} catch (IOException e) {
			e.printStackTrace();
		}
					...
					if (entered!=true) {	
						...
						if(timeOutForMoving == 0) {
							 if(cout<getX()&&getX()-400<cout) {
								...
								//								setImgMoveIndex_Y(0);
								jumpdirection=-1;
								new JumpThread((JumpListener)Tiger.this).start();
