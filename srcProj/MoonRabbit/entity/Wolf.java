...
public class Wolf extends Enemy implements JumpListener {
	// ------------------------------------ Attribute ------------------------------------
	private Image img;
	...
	public Wolf(double x, double y, Canvas instance) {
		super(x, y,instance);
		canvas =instance;
		try {
			img = ImageIO.read(new File("resProj/Character/wolf.png"));
		} catch (IOException e) {
			e.printStackTrace();
		}
		//		setWidth(112);
					...
					if (entered!=true) {	
						...
						if(timeOutForMoving == 0) {
							if(cout<getX()&&getX()-400<cout) {
								...
								//								setImgMoveIndex_Y(0);
								jumpdirection=-1;
								new JumpThread((JumpListener)Wolf.this).start();
						...
						if (timeoutForAttack == 0) {
							...
							timeoutForAttack = rand.nextInt(45)+45;
						}
					}
				}
			}).start();
